...
function ResetPage() {
  ...
  useEffect(() => {
    // Supabase sets a recovery session via the redirect link. Wait for it.
      ...
      {!ready ? (
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <Button disabled={pending} className="h-11 w-full rounded-xl bg-gradient-brand text-white shadow-glow hover:opacity-90 animate-gradient">
            {pending ? <Loader2 className="h-4 w-4 animate-spin" /> : <><ShieldCheck className="h-4 w-4" /> Update password</>}
          </Button>
        </form>
      )}
    </AuthShell>
